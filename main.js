...
// Timer
function criaTimer() { timer = new Date(segundos * 1000); }

// Botão principal
botaoPrincipal.addEventListener("click", function (evento) {
  evento.preventDefault();
  if (paginaAtual === 1) {
    atualizaTimer = setInterval(() => { segundos++; criaTimer(); }, 1000);
  }
  if (paginaAtual === 6) { clearInterval(atualizaTimer); }
});
...
botaoPrincipal.addEventListener("click", function (evento) {
  ...
  if (paginaAtual === 3) respostas[0] = Number(inputRange.value);
  else if (paginaAtual === 4) respostas[1] = Number(inputRange.value);
  else if (paginaAtual === 5) respostas[2] = Number(inputNumber.value);
  else if (paginaAtual === 6) respostas[3] = inputRadioSim.checked ? 1 : 0;
  else if (paginaAtual === 7) respostas[4] = Number(inputRange.value);

  seletorPagina();
});

// Input nome
inputNome.addEventListener("change", function () { nome = inputNome.value; });

// Barra deslizante
labelRange.innerHTML = inputRange.value + "%";
inputRange.addEventListener("input", function () { labelRange.innerHTML = `${inputRange.value}%`; });
...
function seletorPagina() {
  if (paginaAtual === 0) {
    ...
    enunciadoPrincipal.innerHTML += `<p class="descricao-resultado">Você demorou ${adicionaZeroTempo(timer.getMinutes())}:${adicionaZeroTempo(timer.getSeconds())}s para responder esse questionário.</p>`;
...
function calculaResultado() {
  ...
  resultadoFinal = 0;
  for (let i = 0; i < 5; i++) { resultadoFinal += pontos[i]; }
